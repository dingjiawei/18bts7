...
public class SerialUtils{
	...
	private static final String powerStr = "rfid power";
...
//	}
	...
	private boolean isValid(byte[] buffer,int size){
//		byte checkSum = CRC8.calcCrc8(buffer,0,size);
		byte checkSum = CRC8.calcCrc8(buffer,0,4);
		//如果checkSum和传过来的校验码相等返回true，若不相等返回false
		byte abcd = buffer[4];
	...
	private void onDataReceived(final byte[] buffer, final int size){
		String recv;
		//此处对接收的数据进行处理，发送到MainActivity，进行服务器查询
